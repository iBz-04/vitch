...
- Core Vision Transformer models implemented with `tch`
- Typed Rust config structs instead of Python keyword-heavy constructors
- Shared tensor helpers for patching, head splitting, positional embeddings, masking, and window reshaping
- Shape and smoke tests for every ported model
- Small examples for inference and training
- A path to gradually port the wider model collection

The first target is a clean, compiling Rust foundation rather than a complete rewrite of every model at once.
...
1. Create the Rust crate under `rust/`.
2. Implement shared config types and tensor shape helpers.
3. Port the baseline ViT model.
4. Add shape tests equivalent to the existing Python test.
5. Port `SimpleViT`.
6. Port the decorrelation ViT and add a random-data training smoke example.

## Porting Order

The rewrite should move in phases:

1. Core models: `ViT`, `SimpleViT`, `ViT1D`, `ViT3D`
2. Small variants: patch dropout, register tokens, QK norm, normalized ViT, DeepViT
3. Conv and hybrid models: CCT, CvT, PiT, LocalViT, MobileViT, LeViT
4. Window and local attention models: MaxViT, CrossFormer, Twins SVT, RegionViT, SepViT, NesT, XCiT, CaiT
5. Dynamic-token and nested models: NaViT, ATS, nested tensor variants
6. Training and self-supervised wrappers: MAE, SimMIM, MPP, MP3, DINO, distillation, decorrelation
7. Video, audio, and multimodal models: ViViT, VAT, VAAT, video wrappers

High-risk features such as nested tensors, flash attention behavior, FFTs, dynamic masking, and audio preprocessing should be handled after the core crate is stable.

## Validation

Each ported model should have at least one Rust test that verifies the expected output shape. Training-oriented modules should also have a smoke test that produces a scalar loss and runs one backward pass.

For the first phase, success means:
...
During the build, we will stop after each small coherent change and commit before moving on.

Suggested commit boundaries:

- README rewrite
- Rust crate skeleton
- Config and tensor helpers
- Core layers
- Baseline ViT
- First tests
- SimpleViT
- Decorrelation ViT
- Training smoke example

The assistant should remind the user to commit after each boundary. Commits should only be created when explicitly requested.

## Current Status

Status: planning and initial Rust rewrite.

Next step: create the `rust/` crate skeleton and begin the baseline ViT implementation.
